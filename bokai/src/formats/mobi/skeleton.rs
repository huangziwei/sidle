...
impl Skeleton {
    ...
    /// True when the skeleton carries no bytes at all (empty structure and no
    /// chunks). A well-formed KF8 skeleton is never empty; this exists to
    /// satisfy the `len`/`is_empty` pairing.
    pub fn is_empty(&self) -> bool {
        self.skeleton.is_empty() && self.chunks.is_empty()
    }
...
mod tests {
    ...
    #[test]
    fn test_add_aids() {
        use crate::formats::mobi::writer_transform::add_aid_attributes_fast;
