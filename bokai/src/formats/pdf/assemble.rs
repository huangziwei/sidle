...
/// Long edge of each rasterized page, in pixels. ~Kindle Scribe native
/// (1860 × 2480), so the export is visually lossless without ballooning file
/// size.
...
/// Assemble `svgs` (one document per page, in order) into a single multi-page
/// PDF, returning the serialized bytes. Errors if `svgs` is empty or a page
/// fails to parse / rasterize.
pub fn svgs_to_pdf(svgs: &[String]) -> io::Result<Vec<u8>> {
    if svgs.is_empty() {
        return Err(invalid("nothing to render: no pages"));
    }

    let opt = usvg::Options::default();
    let mut doc = Document::with_version("1.5");
    // Reserve the Pages node id up front so each Page can point its /Parent at
    // it before the Pages dict itself exists.
    let pages_id = doc.new_object_id();
    let mut page_ids: Vec<Object> = Vec::with_capacity(svgs.len());

    for (i, svg) in svgs.iter().enumerate() {
        let (rgb, pw, ph, aspect_w, aspect_h) =
            rasterize(svg, &opt).map_err(|e| invalid(format!("render page {}: {e}", i + 1)))?;

        // Flate-compress the raw RGB so a sparse handwriting page (mostly
        // white) stores compactly; the stream is written verbatim under
        // /FlateDecode.
        ...
        let content_id = doc.add_object(Stream::new(
            ...
            content
                .encode()
                .map_err(|e| invalid(format!("encode page content: {e}")))?,
        ));
    ...
    let mut buf = Vec::new();
    doc.save_to(&mut buf)
        .map_err(|e| invalid(format!("serialize PDF: {e}")))?;
    Ok(buf)
}
...
fn rasterize(svg: &str, opt: &usvg::Options) -> io::Result<(Vec<u8>, u32, u32, f32, f32)> {
    let tree = usvg::Tree::from_str(svg, opt).map_err(|e| invalid(format!("parse svg: {e}")))?;
    let size = tree.size();
    ...
    let mut pixmap =
        tiny_skia::Pixmap::new(pw, ph).ok_or_else(|| invalid(format!("alloc pixmap {pw}x{ph}")))?;
    ...
    // tiny_skia stores premultiplied RGBA. Flatten onto the white page:
    // straight-over-white of a premultiplied pixel is `channel + (255 - alpha)`.
    // Fully-opaque ink is unchanged.
