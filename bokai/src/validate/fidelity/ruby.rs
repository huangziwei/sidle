...
pub struct Report {
    ...
    /// bokai dropped or mispaired. In KFX→EPUB, these are pairs bokai fabricated.
    pub only_in_epub: Vec<(RubyPair, usize)>,
    /// Pairs present in KFX but not in EPUB. In EPUB→KFX, these are pairs
    /// bokai fabricated. In KFX→EPUB, these are pairs bokai dropped or mispaired.
...
impl Report {
    ...
    pub fn print_summary(&self, dir: super::Direction) {
        ...
        let (dropped, dropped_n, fabricated, fabricated_n) = if dir.epub_is_source() {
            // EPUB→KFX: EPUB-only = dropped by bokai; KFX-only = fabricated.
            (
                &self.only_in_epub,
                epub_only_total,
                &self.only_in_kfx,
            ...
            // KFX→EPUB: KFX-only = dropped by bokai; EPUB-only = fabricated.
            (
                &self.only_in_kfx,
                kfx_only_total,
                &self.only_in_epub,
...
pub fn extract_pairs_from_kfx(kfx_bytes: &[u8]) -> Result<Vec<RubyPair>, String> {
    ...
    let mut storyline_locs: Vec<&crate::formats::kfx::container::EntityLoc> = Vec::new();
