use bokai::formats::kfx::container::SymbolTable;
...
fn count_nav_entries(entries: &[bokai::formats::kfx::ion::IonValue]) -> usize {
...
fn extract_nav_entries(
    entries: &[bokai::formats::kfx::ion::IonValue],
    extended_symbols: &[String],
    base_symbol_count: usize,
    content_map: &HashMap<String, Vec<String>>,
    fragment_content_map: &HashMap<i64, (String, i64)>,
...
fn report_features(data: &[u8]) -> IonResult<()> {
    ...
    for i in 0..num_entries {
        ...
        if let Ok(value) = parser.parse() {
            ...
            if let bokai::formats::kfx::ion::IonValue::Struct(fields) = &value {
                for (field_id, field_value) in fields {
                    let field_name = resolve_sym(*field_id);

                    if field_name == "features"
                        && let bokai::formats::kfx::ion::IonValue::List(features) = field_value
                    {
                        for (idx, feature) in features.iter().enumerate() {
                            if let bokai::formats::kfx::ion::IonValue::Struct(ffields) = feature {
                                ...
                                for (fid, fval) in ffields {
                                    ...
                                    match fname {
                                        ...
                                        "version_info" => {
                                            ...
                                            if let bokai::formats::kfx::ion::IonValue::Struct(vi) =
                                                fval
                                            {
                                                for (vid, vval) in vi {
                                                    let vname = resolve_sym(*vid);
                                                    if vname == "version"
                                                        && let bokai::formats::kfx::ion::IonValue::Struct(ver) =
                                                            vval
                                                    {
                                                        for (verid, verval) in ver {
                                                            let vername = resolve_sym(*verid);
                                                            if vername == "major_version"
                                                                && let bokai::formats::kfx::ion::IonValue::Int(
                                                                    v,
                                                                ) = verval
                                                            {
                                                                major = *v;
            ...
            if let bokai::formats::kfx::ion::IonValue::Struct(fields) = &value {
                for (field_id, field_value) in fields {
                    ...
                    if field_name == "categorised_metadata"
                        && let bokai::formats::kfx::ion::IonValue::List(categories) = field_value
                    {
                        for category in categories {
                            if let bokai::formats::kfx::ion::IonValue::Struct(cat_fields) = category
                            {
                                ...
                                for (cid, cval) in cat_fields {
                                    ...
                                    match cname {
                                        ...
                                        "metadata" => {
                                            if let bokai::formats::kfx::ion::IonValue::List(items) =
                                                cval
                                            {
                                                for item in items {
                                                    if let bokai::formats::kfx::ion::IonValue::Struct(
                                                        item_fields,
                                                    ) = item
                                                    {
                                                        let mut key = String::new();
                                                        ...
                                                        for (iid, ival) in item_fields {
                                                            ...
                                                            match iname {
                                                                    "key" => {
                                                                        if let bokai::formats::kfx::ion::IonValue::String(s) = ival {
                                                                            key = s.clone();
                                                                        }
                                                                    }
                                                                    "value" => {
            ...
            if let bokai::formats::kfx::ion::IonValue::Struct(fields) = &value {
                for (field_id, field_value) in fields {
                    ...
                    if field_name == "reading_orders"
                        && let bokai::formats::kfx::ion::IonValue::List(orders) = field_value
                    {
                        for (idx, order) in orders.iter().enumerate() {
                            if let bokai::formats::kfx::ion::IonValue::Struct(order_fields) = order
                            {
                                ...
                                for (oid, oval) in order_fields {
                                    ...
                                    match oname {
                                        "reading_order_name" => {
                                            if let bokai::formats::kfx::ion::IonValue::Symbol(s) = oval {
                                                order_name = resolve_sym(*s).to_string();
                                            } else if let bokai::formats::kfx::ion::IonValue::String(s) = oval
                                            {
                                                order_name = s.clone();
                                            }
                                        }
                                        "sections" => {
                                            if let bokai::formats::kfx::ion::IonValue::List(secs) = oval {
                                                for sec in secs {
                                                    if let bokai::formats::kfx::ion::IonValue::Symbol(s) = sec
                                                    {
                                ...
                                if let Some(ppd) = &page_progression_direction {
                                    println!("   page_progression_direction: {}", ppd);
                                }
...
fn format_ion_value_simple<F>(value: &bokai::formats::kfx::ion::IonValue, resolve_sym: &F) -> String
where
    F: Fn(u64) -> String,
{
    use bokai::formats::kfx::ion::IonValue;
            ...
            if let bokai::formats::kfx::ion::IonValue::Struct(fields) = &value {
                ...
                for (field_id, field_value) in fields {
                    ...
                    match field_name.as_str() {
                        ...
                        "page_templates" => {
                            if let bokai::formats::kfx::ion::IonValue::List(tpls) = field_value {
                                for tpl in tpls {
                                    if let bokai::formats::kfx::ion::IonValue::Struct(tpl_fields) =
                                        tpl
                                    {
                                        ...
                                        for (tid, tval) in tpl_fields {
                                            ...
                                            match tname.as_str() {
                                                ...
                                                "fixed_width" => {
                                                    if let bokai::formats::kfx::ion::IonValue::Int(w) = tval {
                                                        dims = format!("{}x", w);
                                                    }
                                                }
                                                "fixed_height" => {
                                                    if let bokai::formats::kfx::ion::IonValue::Int(h) = tval {
                                                        dims = format!("{}{}", dims, h);
                                                    }
                                                }
                                                _ => {}
            ...
            if let bokai::formats::kfx::ion::IonValue::Struct(fields) = &value {
                ...
                for (field_id, field_value) in fields {
                    ...
                    match field_name.as_str() {
                        ...
                        "location" => {
                            if let bokai::formats::kfx::ion::IonValue::String(s) = field_value {
                                location = s.clone();
                            }
                        }
                        "resource_width" => {
                            if let bokai::formats::kfx::ion::IonValue::Int(w) = field_value {
                                width = Some(*w);
                            }
                        }
                        "resource_height" => {
                            if let bokai::formats::kfx::ion::IonValue::Int(h) = field_value {
                                height = Some(*h);
                            }
                        }
                        "thumbnails" => {
            ...
            if let bokai::formats::kfx::ion::IonValue::Struct(fields) = &value {
                ...
                for (field_id, field_value) in fields {
                    ...
                    match field_name.as_str() {
                        ...
                        "content_list" => {
                            if let bokai::formats::kfx::ion::IonValue::List(items) = field_value {
...
fn collect_content_stats<F>(
    items: &[bokai::formats::kfx::ion::IonValue],
    resolve_sym: &F,
...
{
    for item in items {
        if let bokai::formats::kfx::ion::IonValue::Struct(fields) = item {
            ...
            for (field_id, field_value) in fields {
                ...
                match field_name.as_str() {
                    ...
                    "style_events" => {
                        if let bokai::formats::kfx::ion::IonValue::List(events) = field_value {
                            for event in events {
                                if let bokai::formats::kfx::ion::IonValue::Struct(event_fields) =
                                    event
                                {
                    ...
                    "content_list" => {
                        if let bokai::formats::kfx::ion::IonValue::List(nested) = field_value {
                                ...
                                field_value
                        {
                            for (cid, cval) in content_fields {
                                if resolve_sym(*cid) == "text"
                                    && let bokai::formats::kfx::ion::IonValue::String(t) = cval
                                {
...
fn report_locations(data: &[u8]) -> IonResult<()> {
    ...
    fn extract_content_items(
        ...
        if let bokai::formats::kfx::ion::IonValue::Struct(fields) = value {
            ...
            let mut content_index: Option<i64> = None;
            let mut nested_content: Option<&bokai::formats::kfx::ion::IonValue> = None;

            for (fid, fval) in fields {
                let fname = resolve_sym(*fid);
                match fname.as_str() {
                    "id" => {
                        if let bokai::formats::kfx::ion::IonValue::Int(id) = fval {
                            item_id = Some(*id);
                        }
                    }
                    "type" => {
                        if let bokai::formats::kfx::ion::IonValue::Symbol(s) = fval {
                            item_type = resolve_sym(*s);
                        }
                    }
                    "content" => {
                        if let bokai::formats::kfx::ion::IonValue::Struct(cf) = fval {
                            for (cid, cval) in cf {
                                let cname = resolve_sym(*cid);
                                match cname.as_str() {
                                    "name" => {
                                        if let bokai::formats::kfx::ion::IonValue::Symbol(s) = cval
                                        {
                                            content_ref = Some(resolve_sym(*s));
                                        }
                                    }
                                    "index" => {
                                        if let bokai::formats::kfx::ion::IonValue::Int(idx) = cval {
            ...
            // Recurse into nested content_list
            if let Some(bokai::formats::kfx::ion::IonValue::List(items)) = nested_content {
                for item in items {
    ...
    for i in 0..num_entries {
        ...
        if type_idnum == position_map_type {
                    ...
                    for section in sections {
                        if let bokai::formats::kfx::ion::IonValue::Struct(sec_fields) = section {
                            ...
                            for (sid, sval) in sec_fields {
                                ...
                                match fname.as_str() {
                                    "section_name" => {
                                        if let bokai::formats::kfx::ion::IonValue::Symbol(s) = sval
                                        {
            ...
            && let bokai::formats::kfx::ion::IonValue::Struct(fields) = &value
        {
            let mut story_name = String::new();
            let mut content_list: Option<&bokai::formats::kfx::ion::IonValue> = None;
            ...
            if let Some(bokai::formats::kfx::ion::IonValue::List(items)) = content_list {
                for item in items {
            ...
            && let bokai::formats::kfx::ion::IonValue::Struct(fields) = &value
        {
            ...
            for (fid, fval) in fields {
                ...
                match fname.as_str() {
                    ...
                    "content_list" => {
                        if let bokai::formats::kfx::ion::IonValue::List(items) = fval {
                            for item in items {
                                if let bokai::formats::kfx::ion::IonValue::String(s) = item {
                                    texts.push(s.clone());
                                }
                            }
                        }
    ...
    for i in 0..num_entries {
        ...
        if let Ok(value) = parser.parse() {
            ...
            if let bokai::formats::kfx::ion::IonValue::List(items) = &value {
                for item in items {
                    if let bokai::formats::kfx::ion::IonValue::Struct(fields) = item {
                        for (fid, field_value) in fields {
                            let fname = resolve_sym(*fid);
                            if fname == "locations"
                                && let bokai::formats::kfx::ion::IonValue::List(locations) =
                                    field_value
                            {
                                for loc in locations {
                                    if let bokai::formats::kfx::ion::IonValue::Struct(loc_fields) =
                                        loc
                                    {
                                        let mut id = 0i64;
                                        let mut offset = 0i64;
                ...
                for (field_id, field_value) in fields {
                    ...
                    match field_name.as_str() {
                        ...
                        "content" => match field_value {
                            bokai::formats::kfx::ion::IonValue::String(s) => content_len = s.len(),
                            bokai::formats::kfx::ion::IonValue::Blob(b) => content_len = b.len(),
                            _ => {}
                        },
                        _ => {}
                    }
