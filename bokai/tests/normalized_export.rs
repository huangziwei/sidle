...
use bokai::Book;
use bokai::export::{EpubConfig, EpubExporter, Exporter, GlobalStylePool, normalize_book};
...
fn test_normalized_export_contains_css_and_source_named_chapters() {
    ...
    let content_sources: Vec<String> = {
        ...
        spine
            .iter()
            .skip(1)
            .filter_map(|e| book.source_id(e.id).map(str::to_string))
            .collect()
    };
    assert!(
        !content_sources.is_empty(),
        "fixture should have content chapters after the cover"
    );
    ...
    assert!(
        ...
        "Normalized EPUB should contain style.css"
    );
    for source in &content_sources {
        let expected = format!("OEBPS/{source}");
        assert!(
