...
/// `bokai::formats::kfx::merge::merge_kfx_zip` produces a self-contained `.kfx` that
/// loads identically to the source bundle. Verifies the merge fast-path used
/// by `bokai convert in.kfx-zip out.kfx`.
#[test]
fn merge_produces_loadable_single_kfx() {
    ...
    let merged_bytes = bokai::formats::kfx::merge::merge_kfx_zip(&zip_path).expect("merge");
    assert!(
