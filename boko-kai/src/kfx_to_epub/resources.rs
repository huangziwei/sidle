...
/// One bundled CSS file from the source EPUB. Used by `link_stylesheet` so
/// every chapter `<head>` gets a `<link rel="stylesheet">` for each one in
/// addition to the synthesized `style.css`.
#[derive(Debug, Clone)]
pub struct ProcessedCss {
    /// Filename under `OEBPS/` (basename of the source path; e.g.
    /// `book-style.css`).
    pub filename: String,
}

/// Iterate every `external_resource` whose `mime == "text/css"` (or whose
/// `path` ends with `.css`), bundle it as an OEBPS file under its source
/// basename, and return the list for chapter-head linking.
///
/// This is the read-side of S4. The write-side (`export::kfx`) stashes the
/// original href as `$249 path` on the external_resource; we recover it here
/// to land each sheet under its source name so `@import "style-reset.css"`
/// rules between sheets resolve.
///
/// Multi-directory layouts collapse to flat OEBPS files: a source
/// `item/style/book-style.css` → `OEBPS/book-style.css`. CSS `url(...)`
/// references that pointed into sibling source directories (e.g. fonts
/// under `../font/`) won't resolve in the bundled output; this matches the
/// fidelity ceiling laid out in the plan.
pub fn process_css(book: &BookData, out: &mut EpubOutput) -> Vec<ProcessedCss> {
    let mut bundled: Vec<ProcessedCss> = Vec::new();
    let Some(resources) = book.by_type.get(&(KfxSymbol::ExternalResource as u64)) else {
        return bundled;
    };
    ...
    for key in keys {
        ...
        let mime = get_field(fields, KfxSymbol::Mime as u64).and_then(|v| v.as_string());
        let source_path = get_field(fields, KfxSymbol::Path as u64).and_then(|v| v.as_string());
        let is_css = mime.is_some_and(|m| m.starts_with("text/css"))
            || source_path.is_some_and(|p| p.to_ascii_lowercase().ends_with(".css"));
        if !is_css {
            continue;
        }

        let location = match get_field(fields, KfxSymbol::Location as u64).and_then(|v| v.as_string()) {
            Some(l) => l.to_string(),
            None => continue,
        };
        let raw_bytes = match book.raw_media.get(&location) {
            Some(b) => b.clone(),
            None => {
                eprintln!("kfx_to_epub: missing bcRawMedia for CSS at {location:?}");
                continue;
            }
        };
        ...
        let filename = source_path
            ...
            .unwrap_or_else(|| format!("{}.css", key));

        // Don't shadow the synthesized `style.css`; rename collisions.
        if filename == "style.css" || !seen_filenames.insert(filename.clone()) {
            let stem = std::path::Path::new(&filename)
                .file_stem()
                .and_then(|s| s.to_str())
                .unwrap_or("source");
            let mut alt = format!("{}-source.css", stem);
            let mut n = 1;
            while !seen_filenames.insert(alt.clone()) {
                alt = format!("{}-source-{}.css", stem, n);
                n += 1;
            }
            out.add_resource(&alt, raw_bytes, "text/css", None, None);
            bundled.push(ProcessedCss { filename: alt });
        } else {
            out.add_resource(&filename, raw_bytes, "text/css", None, None);
            bundled.push(ProcessedCss { filename });
        }
    }
    bundled
}
