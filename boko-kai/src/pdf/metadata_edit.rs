...
//!
//! The PDF sibling of [`crate::kfx::metadata_edit`] and
//! [`crate::epub::metadata_edit`], with the same contract across all three: a
...
//! prefer XMP. Our own consumer (`probe_pdf`) reads `/Info`, so the edit is
//! authoritative for Sidle's own pipeline; full XMP sync is a later tier.
...
mod tests {
    ...
    fn date_is_written_as_a_pdf_date_string() {
        ...
        let doc = crate::pdf::doc::load_pdf(&out).expect("reload");
        let info = doc.trailer.get(b"Info").expect("info ref");
        let dict = crate::pdf::doc::deref(&doc, info)
