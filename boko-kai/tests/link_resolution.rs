...
fn test_kfx_asset_is_binary_media_with_expected_hash() {
    ...
    // 人間失格 has a single asset — the cover JPEG, addressed by id #880.
    assert!(
        book.list_assets().iter().any(|p| p == Path::new("#880")),
        "Expected #880 to be listed as a KFX asset"
    );
    ...
    assert!(
        ...
        "Expected binary media bytes for #880, got Ion metadata payload ({} bytes)",
        bytes.len()
    );
    assert!(
        bytes.starts_with(&[0xFF, 0xD8, 0xFF]),
        "Expected #880 to be a JPEG (cover.jpeg)"
    );
    assert_eq!(bytes.len(), 32316, "cover.jpeg payload size");
    assert_eq!(
        sha1_hex(bytes.as_slice()),
        "2180e85c65eaa649e770645853c1c94bc4f77618",
        "Unexpected SHA-1 for #880 (cover.jpeg)"
    );
}
