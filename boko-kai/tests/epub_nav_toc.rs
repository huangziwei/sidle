//! Regression: EPUB 3 nav TOC handling in EPUB→KFX and the Sidle reader.
//!
...
//!     `<a href>` cross-references to those positions — so boko's e2k KFX emits
//!     no internal `$266` anchor for them. The Sidle reader (KFX→DOM) must still
...
fn titles(entries: &[boko::TocEntry], out: &mut Vec<String>) {
    for e in entries {
...
fn reader_toc_resolves_intra_chapter_anchors_without_kfx_anchor_table() {
    ...
    // each. No content `<a href>` points at the scenes, so boko's e2k KFX emits
...
<h3 id=\"s3\">Scene Three</h3>{filler}\
    ...
    // EPUB → KFX (boko's own e2k output — ships no internal `$266` anchors).
    ...
    // KFX → reader.
    let reader = boko::kfx_to_epub::kfx_to_reader_book(&kfx).expect("reader book");

    // Collect the three scene entries (skip any synthesized cover).
    let mut flat = Vec::new();
    fn walk(pts: &[boko::kfx_to_epub::navigation::NavPoint], out: &mut Vec<(String, String)>) {
        for p in pts {
