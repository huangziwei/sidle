...
mod tests {
    ...
    /// Verifies the parser against the real (uncommitted) device corpus copied
    /// into `artifacts/p0/real_yjr/`. `#[ignore]`d so the default suite stays
    /// green on machines without those files; run with `--ignored` to check.
    #[test]
    #[ignore = "reads uncommitted real device .yjr from artifacts/p0/real_yjr/"]
    fn real_device_corpus() {
        let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../artifacts/p0/real_yjr");
        if !dir.exists() {
        ...
        for entry in std::fs::read_dir(&dir).unwrap() {
            ...
            if path.extension().and_then(|e| e.to_str()) != Some("yjr") {
                continue;
            }
            let name = path.file_name().unwrap().to_string_lossy().into_owned();
            let anns = parse_file(&path).unwrap();
            eprintln!("{name}: {} annotation(s)", anns.len());
            for a in &anns {
                let (s, e) = (a.start().unwrap(), a.end().unwrap());
                eprintln!(
                    "  [{}] eid {}:{} -> {}:{}  lin {}",
                    a.kind.as_str(),
                    s.eid,
            ...
            // The two P0 highlight-bearing books must decode exactly as proven.
            if name.contains("文学少女") {
                assert_eq!(anns.len(), 2);
                assert_eq!(anns[0].kind, Kind::Highlight);
                assert_eq!((anns[0].start().unwrap().eid, anns[0].start().unwrap().offset), (1254, 44));
                assert_eq!((anns[0].end().unwrap().eid, anns[0].end().unwrap().offset), (1257, 68));
                assert_eq!(anns[1].kind, Kind::Bookmark);
                assert_eq!(anns[1].start().unwrap().eid, 1492);
            }
            if name.contains("十角館") {
                assert_eq!(anns.len(), 1);
                assert_eq!(anns[0].kind, Kind::Highlight);
                assert_eq!((anns[0].start().unwrap().eid, anns[0].start().unwrap().offset), (926, 1));
                assert_eq!((anns[0].end().unwrap().eid, anns[0].end().unwrap().offset), (926, 9));
            }
        }
        assert_eq!(total, 3, "expected 3 annotations across the corpus (文学少女 ×2, 十角館 ×1)");
    }
}
