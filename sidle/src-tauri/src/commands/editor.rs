...
fn extract_images(source_path: &str, kind: SourceKind) -> Result<Vec<RawImage>, String> {
    ...
    let out = match kind {
        ...
        SourceKind::Epub => epub_image::epub_extract_images(&bytes)
            ...
            .collect(),
        // The PDF page-XObject walk isn't built; the rail hides the panel for
        // PDF (`panels_for`), so this is unreachable from the UI.
        SourceKind::Pdf => {
            return Err("image extraction isn't supported for PDF sources yet".to_string());
        }
    };
    Ok(out)
}
...
mod tests {
    ...
    fn panels_are_gated_by_source_capability() {
        ...
        for kind in [SourceKind::Kfx, SourceKind::Epub] {
            ...
            assert!(p.contains(&"metadata".to_string()) && p.contains(&"toc".to_string()));
        }
    }
