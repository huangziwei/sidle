//! Book editor — the app-UI surface over boko-kai's KFX and EPUB edit primitives.
//!
//! Targets **KFX-source** books (`kind == "kfx_to_epub"`, the KFX is both source
//! and reader/device file) and **EPUB-source** books (`epub_to_kfx`, the EPUB is
//! the source and the KFX is derived). Each command dispatches on [`SourceKind`]:
//! a KFX edit is a surgical Ion rewrite of the reader/device file itself; an EPUB
//! edit is a surgical OPF/nav/NCX rewrite of the source, after which the derived
//! KFX is regenerated by the conversion queue. `editor_open` reports the source
//! format and an `editable` flag so the UI gates its panels (PDF is read-only).
//!
...
//! `kfx_sha256` device identity; [`commit_edited_epub`] rewrites the source EPUB,
//! and the follow-up reconvert re-freezes `kfx_sha256` via COALESCE — so device
...
/// Which editable source format a book carries, with its on-disk path. The two
/// arms take different surgical-edit and save-seam paths: KFX rewrites Ion in the
/// reader/device file itself; EPUB rewrites the OPF/nav/NCX source and re-derives
/// the KFX via reconvert.
#[derive(Clone, Copy)]
enum SourceKind {
    Kfx,
    Epub,
...
/// Resolve a book's editable source: its format + on-disk path. Errors for a
/// source format the editor can't edit yet (PDF), or when the source file is
/// missing.
fn require_editable_source(row: &BookRow) -> Result<(SourceKind, String), String> {
    match source_format(row.kind.as_deref()).as_str() {
        ...
        other => Err(format!(
            "the editor edits KFX- and EPUB-source books; this is a {other}-source book (coming soon)"
        )),
    }
}
...
pub struct EditorOpen {
    ...
    pub format: String,
    /// True when the source is editable (a KFX- or EPUB-source book with its
    /// source file on disk). PDF sources open the shell read-only for now.
    pub editable: bool,
...
pub async fn editor_open(state: State<'_, AppState>, book_id: i64) -> Result<EditorOpen, String> {
    ...
    // TOC verdict from the *source* bytes (KFX or EPUB — `validate` sniffs the
    // format). Off the async thread — it's a full container/zip parse.
    let toc = match source.map(|(_, path)| path) {
        Some(path) => tokio::task::spawn_blocking(move || compute_toc(&path))
            .await
            .map_err(|e| e.to_string())?,
        None => None,
    };
...
pub async fn editor_toc(
    ...
    let (_, path) = editor_source(&state, book_id).await?;
...
pub async fn editor_set_toc(
    ...
    let p = path.clone();
    tokio::task::spawn_blocking(move || read_toc_detail(&p))
        .await
        .map_err(|e| e.to_string())?
}
...
/// One extracted image, normalized across source formats. `name` is a display /
/// export stem (KFX `resource_name`, or an EPUB member's filename stem).
struct RawImage {
    name: String,
    ext: String,
    width: Option<u32>,
...
fn compute_toc(source_path: &str) -> Option<EditorToc> {
...
fn read_toc_detail(source_path: &str) -> Result<EditorTocDetail, String> {
    let bytes = std::fs::read(source_path).map_err(|e| format!("read {source_path}: {e}"))?;
...
async fn commit_edited_source(
    ...
    match kind {
        ...
        SourceKind::Epub => commit_edited_epub(path, new_bytes).await,
    }
}
...
/// The EPUB save seam. Back up the source `.epub`, atomically replace it, and
/// drop the backup once settled (rolling back on a failed replace). Unlike the
/// KFX seam it touches **no** DB identity: `sha256` (the storage key) and
/// `kfx_sha256` (the on-device file's hash) both stay frozen. The reader/device
/// file is the *derived* KFX, which the caller's `enqueue_reconvert` regenerates
/// — and that path re-freezes `kfx_sha256` via COALESCE, so device-sync identity
/// is preserved exactly as for a KFX-source edit.
async fn commit_edited_epub(epub_path: &str, new_bytes: Vec<u8>) -> Result<(), String> {
